...
function normalizeSettings(input: Partial<AccountSettings> | null | undefined): AccountSettings {
  const gmailSenders = sanitizeGmailSenders(input?.gmailSenders)
  const whatsappSenders = sanitizeWhatsappSenders(input?.whatsappSenders)

  const requestedGmailActive = input?.activeGmailSenderId?.trim() || ''
  const requestedWhatsappActive = input?.activeWhatsappSenderId?.trim() || ''

  const hasGmailActive = gmailSenders.some(sender => sender.id === requestedGmailActive)
  const hasWhatsappActive = whatsappSenders.some(sender => sender.id === requestedWhatsappActive)

  return {
    gmailSenders,
    whatsappSenders,
    activeGmailSenderId: hasGmailActive ? requestedGmailActive : (gmailSenders[0]?.id || ''),
    activeWhatsappSenderId: hasWhatsappActive ? requestedWhatsappActive : (whatsappSenders[0]?.id || '')
  }
}
...
export function getWhatsAppConfigStatus(whatsapp: {
  phoneNumber: string
  accessToken: string
  phoneNumberId: string
  businessId: string
}): { isConfigured: boolean; missingFields: string[] } {
